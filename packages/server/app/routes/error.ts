...
const reverseSourceMap = async function (data: any) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let complPath = path.resolve(`${sourceMapFile}/${project}/${version}`);
      let files = fs.readdirSync(complPath);
      if (!files.length) {
      ...
      let mapFiles = files.filter((file: string) => file.indexOf(".map") > -1);
      // 找到错误出现相应的sourmap文件
      ...
      let matchFile: string | null = mapFiles[0];

      if (matchFile) {
        let data = fs.readFileSync(`${complPath}/${matchFile}`);
        if (!data) {
        ...
        var sourcesPathMap: sourcesPathMap = {};
...
router.post("/lists", async (ctx: Context) => {
  ...
  let { page, pageSize, project, startTime, endTime } =
  ...
  let lists = rows.map(async (item: any) => {
    let sourceRes = await reverseSourceMap(item);
    return {
      id: item.id,
      name: item.name,
      source: sourceRes,
  ...
  let resLists = await Promise.all([...lists]);
